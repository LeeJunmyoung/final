...
public class Chat_Select_Controller {
	...
	public String Chat_Select(ChatRoomDataBean crdb,HttpServletRequest request){
		...
		System.out.println("test::::"+crdb.toString());
	...
	public void setChatRoom(ChatRoomDataBean crdb,HttpServletRequest request){
		...
		if(check_Multi.contains("n")){

		List checklist = new ArrayList<>();
		checklist = dao.select_False_Chat(crdb.getChat_Num());
				
		
		StringTokenizer stokcheck = new StringTokenizer((String)checklist.get(1) , "n");
		
		int room_num = 1;
		int multi_check = 0;
		
		StringTokenizer another_stokcheck = new StringTokenizer(check_Multi, "n");
		...
		stokcheck = new StringTokenizer((String)checklist.get(1) , "n");
		if(multi_check==0){
